...
def main():
    ...
    optimizer = optim.AdamW([
        {'params': model.backbone.parameters(), 'lr': 1e-5}, # Backbone: 1e-5
        {'params': base_params, 'lr': 2e-4}                  # Head: 2e-4
    ], weight_decay=1e-4)
    ...
    for epoch in range(cfg.MAX_EPOCH):
        ...
        if srcc > best_srcc:
            ...
            torch.save(ema_model.module.state_dict(), save_path) # Save EMA weights
            ...
            try:
                ...
                # Note: visualize_grad_cam needs the base model for hooks, or we need to adapt it for EMA wrapper
                # For simplicity, we use the base model here, but ideally should use EMA model if structure allows
