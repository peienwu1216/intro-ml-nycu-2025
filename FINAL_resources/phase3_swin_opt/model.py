...
class SwinSAMPNet(nn.Module):
    def __init__(self, cfg):
        ...
        # 1. Learnable Layout Queries (Step 1 from modify_plan)
        self.num_queries = 16  # Simulate 16 potential composition patterns
    ...
    def forward(self, x, saliency):
        ...
        # 3. Cross Attention (Step 1 from modify_plan)
        ...
        # Cross Attention: 
        # Query looks for regions matching specific composition patterns
        attn_out, _ = self.cross_attn(query=queries, key=feat_seq, value=feat_seq)
        
        # attn_out: [B, 16, 768]
        # Aggregate layout features
        feat_layout = attn_out.mean(dim=1) # [B, 768]
        
        # 4. Heads
        score_dist = self.score_head(feat_layout)
        attrs = self.attr_head(feat_layout)
