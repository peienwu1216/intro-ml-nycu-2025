...
from dataset import CADBDataset
from model import SwinSAMPNet
from loss import EMDLoss, RankLoss, AttributeLoss
from utils import calculate_accuracy, calculate_lcc, calculate_spearmanr, dist2ave, visualize_grad_cam

def train_one_epoch(model, loader, optimizer, emd_loss_fn, rank_loss_fn, attr_loss_fn, device, cfg):
    model.train()
    running_loss = 0.0
...
def validate(model, loader, device, emd_loss_r1, emd_loss_r2):
    ...
    avg_emd_r1 = emd_r1_sum / total_count # Note: SAMPNet sums then divides by N
...
def main():
    ...
    # Model
    model = SwinSAMPNet(cfg).to(device)
    
    # Optimizer (Step 2 from modify_plan: Differential LR)
    ...
    optimizer = optim.AdamW([
        {'params': model.backbone.parameters(), 'lr': cfg.LR * 0.1}, # Backbone: 1e-5
        {'params': base_params, 'lr': cfg.LR}                        # Head: 1e-4
    ...
    emd_loss_r1 = EMDLoss(r=1, reduction='sum') # For validation metric
    emd_loss_r2 = EMDLoss(r=2, reduction='sum') # For validation metric
    rank_loss = RankLoss(margin=0.1)
    ...
    for epoch in range(cfg.MAX_EPOCH):
        ...
        train_loss = train_one_epoch(model, train_loader, optimizer, emd_loss_train, rank_loss, attr_loss, device, cfg)
        
        acc, emd1, emd2, mse, srcc, lcc = validate(model, test_loader, device, emd_loss_r1, emd_loss_r2)
        
        print(f"Train Loss: {train_loss:.4f}")
        print(f"Val Results: Acc={acc:.2%}, EMD1={emd1:.4f}, EMD2={emd2:.4f}, MSE={mse:.4f}, SRCC={srcc:.4f}, LCC={lcc:.4f}")
        
        scheduler.step(train_loss) # Or step on validation metric
