...
class QuarterPattern(nn.Module):
    ...
    def forward(self, x, s):
        # MPS workaround for adaptive_avg_pool2d
        if x.device.type == 'mps':
            feat = F.adaptive_avg_pool2d(x.cpu(), output_size=(2,2)).to(x.device)
        else:
...
class ThirdOfRulePattern(nn.Module):
    ...
    def forward(self, x, s):
        # MPS workaround for adaptive_avg_pool2d
        if x.device.type == 'mps':
            feat = F.adaptive_avg_pool2d(x.cpu(), output_size=(3,3)).to(x.device)
        else:
...
class VerticalThirdPattern(nn.Module):
    ...
    def forward(self, x, s):
        # MPS workaround for adaptive_avg_pool2d
        if x.device.type == 'mps':
            feat = F.adaptive_avg_pool2d(x.cpu(), output_size=(3,1)).to(x.device)
        else:
...
class HorThirdPattern(nn.Module):
    ...
    def forward(self, x, s):
        # MPS workaround for adaptive_avg_pool2d
        if x.device.type == 'mps':
            feat = F.adaptive_avg_pool2d(x.cpu(), output_size=(1,3)).to(x.device)
        else:
