...
class Trainer(object):
    def __init__(self, model, cfg):
        self.cfg = cfg
        self.model = model
        self.device = torch.device('cuda:{}'.format(self.cfg.gpu_id) if torch.cuda.is_available() else 'mps' if torch.backends.mps.is_available() else 'cpu')
    ...
    def write2csv(self):
        csv_path = os.path.join(self.cfg.log_dir, '{}.csv'.format(self.cfg.exp_name))
...
if __name__ == '__main__':
    ...
    log_file = os.path.join(cfg.log_dir, f"{current_time}_log.txt")
    sys.stdout = Logger(log_file)

    device = torch.device('cuda:{}'.format(cfg.gpu_id) if torch.cuda.is_available() else 'mps' if torch.backends.mps.is_available() else 'cpu')
    # evaluate(cfg)
